...
import ThmlSVG from '@/images/tonghua-media-lab.svg';

type TongHuaMediaLabProps =
  PageProps<GatsbyTypes.TongHuaMediaLabPageQueryQuery>;

const TongHuaMediaLab: React.FC<TongHuaMediaLabProps> = ({ data }) => {
  const { allBusinessesJson } = data;
  const businesses = filter(
    allBusinessesJson.edges,
    (x) => x.node.key !== 'thl',
  );
  const thlSlides: string[] = filter(
    allBusinessesJson.edges,
    (x) => x.node.key === 'thl',
...
export default TongHuaMediaLab;

export const query = graphql`
  query TongHuaMediaLabPageQuery {
    allBusinessesJson {
      edges {
        node {
          title
